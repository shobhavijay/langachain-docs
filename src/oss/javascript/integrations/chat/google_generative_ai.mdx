...
<Note>
    **This library will be deprecated**
    ...
    be replaced by the [ChatGoogle](/oss/integrations/chat/google) library.
    New implementations should use the [ChatGoogle](/oss/integrations/chat/google) library instead and
...
Tool calling with Google AI is mostly the same [as tool calling with other models](/oss/langchain/tools), but has a few restrictions on schema.
